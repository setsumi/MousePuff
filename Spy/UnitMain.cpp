...
void __fastcall TFormMain::TimerPollTimer(TObject* /* Sender */)
{
	...
	if (wnd == Handle)
	{ // redetect on activate self and then go back
		prevWnd = NULL;
	}
	if (wnd && wnd != Handle && wnd != prevWnd)
	{
		...
		RECT rect =
		{0};
		if (S_OK == DwmGetWindowAttribute(wnd, DWMWA_EXTENDED_FRAME_BOUNDS, &rect,
			sizeof(RECT)))
		{
			UnicodeString str;
			str.sprintf(L"%d, %d (%d, %d)", rect.left, rect.top, rect.right - rect.left,
